/* 
 * Disassembly code for the PIC 16F84 microcontroller.
 * 
...
char *disassemble_16f84 (unsigned short instr, unsigned int options)
{
    ...
    switch (INSTR_GROUP_14(instr)) {
	  ...
	  if ((options & FSR_NAMES) == FSR_NAMES
	   && BIT_GET_F_14(instr) <= MAX_INDEX) {
	      sprintf(instr_desc, "%-8s  %s, %d",
		      get_mnemonic(instr, BIT_INSTR),
		      get_fsr_name(BIT_GET_F_14(instr)),
		      BIT_GET_B_14(instr));
	  } else {
...
static char *get_mnemonic(unsigned short instr, int type)
{
	...
	switch ((instr & 0x3F00) >> 8) {
	  ...
	  case 0x01: {
	      if (BYTE_GET_D_14(instr)) { /* File reg. or W for destination? */
		  return "clrf";
	      } else {
		  return "clrw";
	      }
